...
describe("items linked to an exam on the calendar", () => {
  it("creates one the first time the exam is saved", async () => {
    const s = await store({ courses: [courseRow()] });
    await s.linkExamItem({
      eventId: "ev-1",
      courseId: "c1",
      categoryId: "cat-1",
      title: "Midterm",
    ...
    expect(stored("grade_items")).toHaveLength(1);
    expect(stored("grade_items")[0]).toMatchObject({
      event_id: "ev-1",
      max_score: 50,
      score: null,
    });
  ...
  it("updates the same one when the exam is edited", async () => {
    const s = await store({ courses: [courseRow()] });
    await s.linkExamItem({
      eventId: "ev-1",
      courseId: "c1",
  ...
  it("does not wipe a score that was already entered", async () => {
    const s = await store({
      grade_items: [itemRow({ event_id: "ev-1", score: 44 })],
    });
    await s.linkExamItem({
      eventId: "ev-1",
      courseId: "c1",
    ...
    expect(stored("grade_items")[0].score).toBe(44);
  });

  it("removes the item when the exam is deleted", async () => {
    const s = await store({
      grade_items: [itemRow({ event_id: "ev-1" })],
    });
    await s.unlinkExamItem("ev-1");
    expect(stored("grade_items")).toHaveLength(0);
  });

  it("does nothing for an event that never had an item", async () => {
    const s = await store({ grade_items: [itemRow()] });
    await s.unlinkExamItem("ev-9");
    expect(stored("grade_items")).toHaveLength(1);
  });
});
